...
(ns viz2
  (:import [com.tinkerpop.blueprints.oupls.jung GraphJung]
           [edu.uci.ics.jung.visualization VisualizationViewer GraphZoomScrollPane]
           [edu.uci.ics.jung.graph DelegateTree Forest Tree]
           [edu.uci.ics.jung.visualization.decorators ToStringLabeller EdgeShape$Line]
           [edu.uci.ics.jung.algorithms.layout TreeLayout RadialTreeLayout SpringLayout]
...
;; (defn forest [context]
;;   (proxy [Forest GraphJung] []
;;     (getChildCount [vextex])
;;     (getChildEdges [vextex])
;;     (getChildren   [vextex])
;;     (getParent     [vextex])
;;     (getParentEdge [vextex])
;;     (getTrees      [])))

;; Tree
;; (getDepth [vertex]) (getHeight[]) (getRoot[])

(defn tree-frame [context]
  (let [graph-jung (GraphJung. (tdb/jung (:db context)))
        graph  (doto (DelegateTree. graph-jung)
                 (.setRoot (:user context)))
                
        ;graph  )
        treeLayout  (TreeLayout. graph)
        radialLayout (doto (RadialTreeLayout. graph)
                       (.setSize (Dimension. 600 600)))
        vv (doto ( VisualizationViewer. treeLayout  (Dimension. 600 600))
             (.setBackground Color/white)
             (.setVertexToolTipTransformer (ToStringLabeller.)))]
    (doto (. vv getRenderContext)
      (.setEdgeShapeTransformer (EdgeShape$Line.))
      (.setVertexLabelTransformer (ToStringLabeller. ))
      (.setArrowFillPaintTransformer (ConstantTransformer. Color/lightGray)))
    (GraphZoomScrollPane. vv)))



(defn spring-frame [context]
  (let [graph-jung (GraphJung. (tdb/jung (:db context)))
        layout (SpringLayout. graph-jung)
        vv (doto ( VisualizationViewer. layout  (Dimension. 600 600))
             (.setBackground Color/white)
             (.setVertexToolTipTransformer (ToStringLabeller.)))]
    (doto (. vv getRenderContext)
      (.setEdgeShapeTransformer (EdgeShape$Line.))
